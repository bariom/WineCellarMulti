...
export function wineToDraft(wine: Wine): WineDraft {
  return {
    ...
    reorder_enabled: wine.reorder_enabled !== false,
    commercial_status: wine.commercial_status || "active",
    ...
    storage_location_id: wine.storage_allocations?.[0]?.location_id || "",
    storage_bin_id: wine.storage_allocations?.[0]?.bin_id || "",
...
export function draftPayload(draft: WineDraft) {
  ...
  return {
    ...
    reorder_enabled: draft.reorder_enabled,
    ...
    storage_allocations: Number(draft.quantity || 0) > 0 ? [{ location_id: draft.storage_location_id || null, bin_id: draft.storage_bin_id || null, quantity: Number(draft.quantity || 0) }] : [],
...
export function wishlistToDraft(item: WishlistItem): WishlistDraft {
  return {
    ...
    investment_amount: item.investment_amount === null ? "" : String(item.investment_amount),
...
export function wishlistPayload(draft: WishlistDraft) {
  return {
    ...
    investment_amount: draft.investment_amount === "" ? null : Number(draft.investment_amount),
    currency: draft.currency.trim().toUpperCase() || "CHF",
    merchant: draft.merchant.trim(),
