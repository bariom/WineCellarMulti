...
export function WineStrategySection({ wine, locale, canWrite, onChanged, onAllocatedChange, embedded = false }: {
  ...
  return <details className={`detail-section wine-strategy-section${embedded ? " wine-strategy-section--embedded" : ""}`} data-wine-detail-section="04" tabIndex={-1} open={embedded || undefined} onToggle={(event) => {
    ...
    {editing ? <div className="wine-strategy-editor">
      ...
      {error ? <p className="form-error" role="alert">{error}</p> : null}
      <div className="form-actions"><button type="button" className="secondary" disabled={saving} onClick={() => { setEditing(false); setError(""); }}>{locale === "it" ? "Annulla" : "Cancel"}</button><button type="button" disabled={saving} onClick={() => void save()}>{saving ? (locale === "it" ? "Salvo…" : "Saving…") : (locale === "it" ? "Salva obiettivo" : "Save purpose")}</button></div>
    </div> : <>
