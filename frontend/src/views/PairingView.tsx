...
type BuyingAdviceResult = {
  ...
  recommendations: Array<{
    name: string; producer: string; vintage: string; merchant: string;
    merchant_type: "local_shop" | "online"; price: string; currency: string;
    availability: string; delivery_estimate: string; source_url: string;
    reason: string; local: boolean; confidence: "high" | "medium" | "low";
  }>;
  estimated_cost_usd: string;
};
        ...
        <form className="pairing-form buying-advice-form" onSubmit={onGenerateBuyingAdvice}>
          <div className="buying-advice-fields">
            <label>
              ...
              <select value={buyingPurpose} onChange={(event) => setBuyingPurpose(event.target.value as typeof buyingPurpose)} disabled={!canGenerateAi || generatingAi === "buying-advice"}>
            ...
            <label>
              ...
              <select value={buyingNeededBy} onChange={(event) => setBuyingNeededBy(event.target.value as typeof buyingNeededBy)} disabled={!canGenerateAi || generatingAi === "buying-advice"}>
            ...
            <label>
              ...
              <input value={buyingLocation} onChange={(event) => setBuyingLocation(event.target.value)} placeholder={locale === "it" ? "Es. Lugano, Svizzera" : "E.g. Lugano, Switzerland"} disabled={!canGenerateAi || generatingAi === "buying-advice"} />
            </label>
            <label>
              <span>{locale === "it" ? "Budget massimo per bottiglia (CHF)" : "Maximum price per bottle (CHF)"}</span>
              <input type="number" min="1" step="1" value={buyingMaxPrice} onChange={(event) => setBuyingMaxPrice(event.target.value)} placeholder="80" disabled={!canGenerateAi || generatingAi === "buying-advice"} />
            </label>
          </div>
          {buyingPurpose === "pairing" ? (
            <label>
              <span>{locale === "it" ? "Con cosa vuoi abbinarlo?" : "What are you pairing it with?"}</span>
              <input value={buyingPairingWith} onChange={(event) => setBuyingPairingWith(event.target.value)} placeholder={locale === "it" ? "Es. brasato, sushi, formaggi" : "E.g. braised beef, sushi, cheese"} disabled={!canGenerateAi || generatingAi === "buying-advice"} />
            </label>
          ) : null}
          <label>
            <span>{locale === "it" ? "Altri criteri" : "Other criteria"}</span>
            <textarea rows={2} value={buyingPreferences} onChange={(event) => setBuyingPreferences(event.target.value)} placeholder={locale === "it" ? "Es. rosso, Piemonte, poco legno, regalo..." : "E.g. red, Piedmont, low oak, gift..."} disabled={!canGenerateAi || generatingAi === "buying-advice"} />
          </label>
          ...
          <button type="submit" disabled={!canGenerateAi || generatingAi === "buying-advice"}>
            <ButtonBusyContent busy={generatingAi === "buying-advice"} idleLabel={locale === "it" ? "Cerca vini da acquistare" : "Find wines to buy"} busyLabel={t("generating")} />
          </button>
          {generatingAi === "buying-advice" ? <LoadingState label={t("generating")} compact /> : null}
        </form>
        {buyingAdviceResult ? (
