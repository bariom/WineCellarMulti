...
from sqlalchemy import DateTime, ForeignKey, JSON, Numeric, String, Text, Uuid, UniqueConstraint
...
class CoOwnershipParticipant(Base):
    ...
    __table_args__ = (UniqueConstraint("agreement_id", "email", name="uq_coownership_participant_email"),)

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    agreement_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("coownership_agreements.id", ondelete="CASCADE"), index=True)
