...
class WineCatalogAlias(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    catalog_entry_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("wine_catalog_entries.id", ondelete="CASCADE"), index=True)
...
class WineRecognitionLog(Base):
    ...
    matched_catalog_entry_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("wine_catalog_entries.id", ondelete="SET NULL"), nullable=True)
