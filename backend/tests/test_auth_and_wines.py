...
def test_cellar_intelligence_ai_respects_quantitative_purposes(monkeypatch):
    ...
    assert captured_request["max_output_tokens"] == 4096
...
def test_wine_product_photo_upload_serves_two_private_sizes_and_deletes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda content, model, timeout, idle: ai_png,
    )
    monkeypatch.setattr(wines_route, "warm_bottle_photo_worker", lambda model, idle: None)
    monkeypatch.setattr(settings, "wine_photo_ai_enabled", True)
    warmed = client.post("/api/v1/wines/photo/warmup")
    assert warmed.status_code == 204
...
def test_cellar_ai_command_proposes_and_confirms_wines_below_value_threshold(monkeypatch):
    from app.api.routes import ai as ai_routes

    client = TestClient(app)
    assert register(client).status_code == 201
    current_value_wine = client.post(
        "/api/v1/wines",
        json={
            "name": "Bianco Quotidiano",
            "producer": "Cantina Uno",
            "vintage": "2023",
            "quantity": 3,
            "price": 25,
            "current_value": 35,
    ...
    purchase_value_wine = client.post(
        "/api/v1/wines",
        json={
            "name": "Rosso Quotidiano",
            "producer": "Cantina Due",
            "vintage": "2022",
            "quantity": 1,
            "price": 30,
            "currency": "CHF",
    ...
    prepared = client.post(
        ...
        json={
            "request_id": request_id,
            "text": "Considera bottiglie a meno di 40 franchi come da bere.",
            "locale": "it",
            "timezone": "Europe/Zurich",
        },
    )
    ...
    assert confirmed.status_code == 200, confirmed.text
    assert confirmed.json()["status"] == "executed"
    first_allocations = client.get(
        f"/api/v1/intelligence/wines/{current_value_wine['id']}/allocations"
    ).json()
    assert sorted((item["purpose"], item["quantity"]) for item in first_allocations) == [
        ("drink", 1),
        ("drink", 1),
        ("investment", 1),
    ]
...
def test_cellar_ai_falls_back_to_common_action_variants_when_ai_is_uncertain():
    ...
    commands = {
        ...
        "Ho ritirato Sassicaia 2022.": "collect_wine",
        "Ho ritirato i vini da Enoteca Rossi.": "collect_wine",
        "Sono passato a ritirare le bottiglie da Arvi.": "collect_wine",
        "I picked up Sassicaia 2022.": "collect_wine",
        "I collected my order from Enoteca Rossi.": "collect_wine",
        "Mark my pickup as collected and move it to my cellar.": "collect_wine",
